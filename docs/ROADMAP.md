...
- Add reconnect-safe turn recovery so disconnected players can return without freezing the room.
- Add optional bot replacement for disconnected players after a configurable timeout.

## Security

- Move production session storage from `MemoryStore` to Redis or Mongo-backed storage.
...
- Preserve the current guest-first flow with no forced account system.
