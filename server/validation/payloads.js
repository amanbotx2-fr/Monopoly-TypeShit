...
function stringField(value, label, { min = 1, max = 255, pattern = null, optional = false } = {}) {
    ...
    if (pattern && !pattern.test(trimmed)) return fail('bad-string-format', `${label} has invalid format`);
    return ok(trimmed);
}
...
function finiteNumber(value, label, { min = 0, max = Number.MAX_SAFE_INTEGER, integer = true, optional = false } = {}) {
    if (value === undefined && optional) return ok(undefined);
    if (typeof value !== 'number' || !Number.isFinite(value)) {
        return fail('bad-number', `${label} must be a finite number`);
    }
    if (integer && !Number.isInteger(value)) return fail('bad-integer', `${label} must be an integer`);
...
function noPayload(args, event) {
    ...
    if (args.length === 2 && args[0] === undefined && typeof args[1] === 'function') return ok(undefined);
...
function validateSocketAuth(payload, tokenColors) {
    ...
    const fields = rejectUnknownKeys(root.value, ['roomCode', 'username', 'color', 'asSpectator'], 'socket auth');
    ...
    return ok({ roomCode: code.value, username: name.value, color: color.value, asSpectator: !!asSpectator.value });
}
...
function validateRulesPayload(payload) {
    ...
    for (const key of Object.keys(rulesObj.value)) {
        ...
        else r = finiteNumber(rulesObj.value[key], key, { min: spec.min, max: spec.max, integer: spec.kind === 'int' });
        if (!r.ok) return r;
        rules[key] = r.value;
    }
    return ok({ rules });
...
function validateAuctionBidPayload(payload, player, room) {
    ...
    if (!fields.ok) return fields;
    const amount = finiteNumber(root.value.amount, 'amount', { min: 1, max: Math.min(player.cash, MAX_MONEY), integer: true });
...
function normalizeBundle(bundle, label) {
    ...
    const cash = finiteNumber('cash' in root.value ? root.value.cash : 0, `${label}.cash`, { min: 0, max: MAX_MONEY, integer: true });
    if (!cash.ok) return cash;

    const rawProps = 'properties' in root.value ? root.value.properties : [];
    if (!Array.isArray(rawProps)) return fail('bad-properties', `${label}.properties must be an array`);
    ...
    for (const raw of rawProps) {
        ...
        if (!pos.ok) return pos;
        if (seen.has(pos.value)) return fail('duplicate-property', `${label}.properties contains duplicates`);
        seen.add(pos.value);
    ...
    const chance = finiteNumber('chance' in cards.value ? cards.value.chance : 0, `${label}.jailCards.chance`, { min: 0, max: 2, integer: true });
    if (!chance.ok) return chance;
    const chest = finiteNumber('chest' in cards.value ? cards.value.chest : 0, `${label}.jailCards.chest`, { min: 0, max: 2, integer: true });
    if (!chest.ok) return chest;

    return ok({ cash: cash.value, properties, jailCards: { chance: chance.value, chest: chest.value } });
}

function validateBundleAssets(room, player, bundle, label) {
    if (!player || player.bankrupt) return fail('bad-player', `${label} owner is not an active player`);
    ...
    for (const pos of bundle.properties) {
        ...
        if (state.owner !== player.userId) return fail('bad-property-owner', `${label}.properties contains a property not owned by the player`);
    }
...
function validateTradePayload(payload, room, actor, { requireRecipient = false } = {}) {
    ...
    if (!root.ok) return root;
    const allowed = requireRecipient ? ['toUserId', 'offer', 'request'] : ['tradeId', 'offer', 'request'];
...
function validateBankruptPayload(payload, room, player) {
    ...
    if (!fields.ok) return fields;
    if (room.turnPhase !== 'resolving') return fail('bad-bankruptcy-state', 'bankruptcy is only allowed while resolving debt');
    ...
    if (!pending || pending.userId !== player.userId) return fail('bad-bankruptcy-state', 'no pending debt for this player');
    const resources = availableBankruptcyResources(room, player);
    if (resources >= pending.amount) {
        return fail('bad-bankruptcy-state', 'player can cover pending debt with available resources', { resources, debt: pending.amount });
    }
...
function availableBankruptcyResources(room, player) {
    ...
    for (const pos of player.owned) {
        ...
        if (!state.mortgaged && ['property', 'station', 'utility'].includes(tile.type)) {
            total += Number.isFinite(tile.mortgage) ? tile.mortgage : Math.floor((tile.price || 0) / 2);
        }
    }
    return total;
}
...
function validateBoardTiles(tiles) {
    ...
    for (let i = 0; i < tiles.length; i++) {
        ...
        const fields = rejectUnknownKeys(tile, ['pos', 'type', 'name', 'group', 'color', 'price', 'rent', 'houseCost', 'mortgage', 'amount'], `tile ${i}`);
        if (!fields.ok) return fields;
        ...
        if (type.value === 'property') {
            const group = stringField(tile.group, `tile ${i}.group`, { min: 1, max: 32, pattern: /^[A-Za-z0-9_-]+$/ });
            if (!group.ok) return group;
            if (!Array.isArray(tile.rent) || tile.rent.length !== 6) return fail('bad-board', `tile ${i}.rent must contain six values`);
            const rent = [];
            for (let j = 0; j < 6; j++) {
                const r = finiteNumber(tile.rent[j], `tile ${i}.rent[${j}]`, { min: 0, max: MAX_MONEY, integer: true });
                if (!r.ok) return r;
                rent.push(r.value);
            }
            const price = finiteNumber(tile.price, `tile ${i}.price`, { min: 0, max: MAX_MONEY, integer: true });
            if (!price.ok) return price;
            const houseCost = finiteNumber(tile.houseCost, `tile ${i}.houseCost`, { min: 0, max: MAX_MONEY, integer: true });
            if (!houseCost.ok) return houseCost;
            const mortgage = finiteNumber('mortgage' in tile ? tile.mortgage : Math.floor(price.value / 2), `tile ${i}.mortgage`, { min: 0, max: MAX_MONEY, integer: true });
        ...
        if (type.value === 'tax') {
            const amount = finiteNumber(tile.amount, `tile ${i}.amount`, { min: 0, max: MAX_MONEY, integer: true });
            if (!amount.ok) return amount;
...
function validateGroupColors(value) {
    ...
    for (const [key, val] of entries) {
        if (!/^[A-Za-z0-9_-]{1,32}$/.test(key)) return fail('bad-group-colors', 'group color key has invalid format');
        if (typeof val !== 'string' || !HEX_COLOR_RE.test(val)) return fail('bad-group-colors', 'group color value must be a hex color');
        out[key] = val;
    }
    return ok(out);
}
...
function validateCreateBoardBody(payload) {
    ...
    if (!root.ok) return root;
    const fields = rejectUnknownKeys(root.value, ['id', 'name', 'tiles', 'groupColors', 'description', 'isPublic', 'authorUsername'], 'board');
    if (!fields.ok) return fields;
    ...
    const description = stringField('description' in root.value ? root.value.description : '', 'description', { min: 0, max: MAX_DESCRIPTION });
    if (!description.ok) return description;
    const isPublic = booleanField(root.value.isPublic, 'isPublic', { optional: true });
    if (!isPublic.ok) return isPublic;
    const authorUsername = username(root.value.authorUsername, 'authorUsername', { optional: true });
...
function validatePatchBoardBody(payload) {
    ...
    if (!root.ok) return root;
    const fields = rejectUnknownKeys(root.value, ['name', 'tiles', 'groupColors', 'description', 'isPublic'], 'board patch');
    if (!fields.ok) return fields;
    const out = {};
    ...
    if ('description' in root.value) {
        const description = stringField(root.value.description, 'description', { min: 0, max: MAX_DESCRIPTION });
        if (!description.ok) return description;
...
function validateDuplicateBoardBody(payload) {
    ...
    if (!root.ok) return root;
    const fields = rejectUnknownKeys(root.value, ['name', 'description', 'isPublic'], 'duplicate board');
    if (!fields.ok) return fields;
    const out = {};
    if ('name' in root.value) {
        const name = stringField(root.value.name, 'name', { min: 1, max: MAX_NAME });
...
function validateCreateRoomBody(payload, tokenColors) {
    ...
    if (!root.ok) return root;
    const fields = rejectUnknownKeys(root.value, ['username', 'color', 'boardId', 'customBoardId'], 'room');
    if (!fields.ok) return fields;
    const name = username(root.value.username);
    if (!name.ok) return name;
    const color = tokenColor(root.value.color, tokenColors);
    ...
    return ok({ username: name.value, color: color.value, boardId: b.value, customBoardId: custom.value });
}
