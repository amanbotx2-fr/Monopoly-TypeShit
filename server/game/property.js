...
function mortgage(room, player, pos) {
	...
	if (def.type === 'property' && st.houses > 0)
		return { ok: false, error: 'sell-buildings-first' };
	...
	if (def.type === 'property') {
		for (let i = 0; i < 40; i++) {
			const d = tileDef(room, i),
				s = tileSt(room, i);
			if (
				d.type === 'property' &&
				d.group === def.group &&
				s.owner === player.userId &&
				s.houses > 0
			) {
...
function unmortgage(room, player, pos) {
	...
	return {
		ok: true,
		events: r.events.concat({ type: 'unmortgage', userId: player.userId, pos }),
	};
}
...
function buildHouse(room, player, pos) {
	...
	if (!r.ok) {
		/* we already deducted bank supply; rollback */
		if (st.houses === 5) {
			room.bank.hotels += 1;
			room.bank.houses -= 4;
			st.houses = 4;
	...
	return {
		ok: true,
		events: r.events.concat({ type: 'build', userId: player.userId, pos, houses: st.houses }),
	};
}
