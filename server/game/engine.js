...
function rollDice(room) {
	if (room && room._devDice) {
		const dice = room._devDice;
		room._devDice = null;
		return dice;
	}
...
// ─── Ownership helpers ───────────────────────────────────────────────────────
function tileDef(room, pos) {
	return room.board.tiles[pos];
}
function tileSt(room, pos) {
...
function advanceTo(room, player, targetPos, { passGo = true, animate = true } = {}) {
	...
	while (pos !== targetPos) {
		...
		if (pos === 0 && passGo) {
			const salary =
				room.rules.doubleOnGo && targetPos === 0
					? room.rules.salary * 2
					: room.rules.salary;
...
function moveBy(room, player, delta, opts) {
	...
	return advanceTo(room, player, target, {
		...opts,
		passGo: delta > 0 && opts?.passGo !== false,
	});
}
...
function payJailFine(room, player) {
	...
	return {
		ok: true,
		events: r.events.concat({ type: 'jail-escape', userId: player.userId, method: 'fine' }),
	};
}
...
function buyCurrent(room, player) {
	...
	return {
		ok: true,
		events: [{ type: 'buy', userId: player.userId, pos: def.pos, price: def.price }],
	};
}
...
function declineBuy(room, player) {
	...
	return {
		ok: true,
		events: [{ type: 'decline-buy', userId: player.userId, pos: player.position }],
	};
}
