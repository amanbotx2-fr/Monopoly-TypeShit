...
function updateTrade(room, actor, tradeId, offer, request) {
	...
	if (actor.userId !== tr.fromUserId && actor.userId !== tr.toUserId)
		return { ok: false, error: 'not-party' };
...
function executeTrade(room, tr) {
	...
	for (const pos of offerProps)
		if (room.tileState[pos].owner !== from.userId)
			return { ok: false, error: 'from-not-owner' };
	for (const pos of requestProps)
		if (room.tileState[pos].owner !== to.userId) return { ok: false, error: 'to-not-owner' };

	// Jail cards
	const fromLg = room.jailFreeLedger[from.userId] || { chance: 0, chest: 0 };
	const toLg = room.jailFreeLedger[to.userId] || { chance: 0, chest: 0 };
	if (fromLg.chance < tr.offer.jailCards.chance || fromLg.chest < tr.offer.jailCards.chest)
		return { ok: false, error: 'from-no-card' };
	if (toLg.chance < tr.request.jailCards.chance || toLg.chest < tr.request.jailCards.chest)
		return { ok: false, error: 'to-no-card' };

	const events = [];

	// Cash swap.
	if (tr.offer.cash > 0)
		events.push(...transfer(room, from.userId, to.userId, tr.offer.cash, 'trade').events);
	if (tr.request.cash > 0)
		events.push(...transfer(room, to.userId, from.userId, tr.request.cash, 'trade').events);
...
function prune(room) {
	...
	room.trades = room.trades
		...
		.concat(room.trades.filter((t) => t.status !== 'open').slice(-10));
}
