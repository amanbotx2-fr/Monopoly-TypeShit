...
const CHANCE = [
	...
	{
		id: 'ch_advance_london',
		text: 'Advance to London. If you pass GO, collect $200.',
		effect: { kind: 'moveTo', pos: 37, passGo: true },
	},
	{
		id: 'ch_advance_paris',
		text: 'Advance to Paris. If you pass GO, collect $200.',
		effect: { kind: 'moveTo', pos: 34, passGo: true },
	},
	{
		id: 'ch_advance_barcelona',
		text: 'Advance to Barcelona. If you pass GO, collect $200.',
		effect: { kind: 'moveTo', pos: 14, passGo: true },
	},
	{
		id: 'ch_advance_istanbul',
		text: 'Take a trip to Istanbul. Pass GO, collect $200.',
		effect: { kind: 'moveTo', pos: 5, passGo: true },
	},
	{
		id: 'ch_nearest_station_1',
		text: 'Advance to the nearest station. If unowned, you may buy it. If owned, pay the owner double rent.',
		effect: { kind: 'moveToNearest', target: 'station', rentMult: 2 },
	},
	{
		id: 'ch_nearest_station_2',
		text: 'Advance to the nearest station. If unowned, you may buy it. If owned, pay the owner double rent.',
		effect: { kind: 'moveToNearest', target: 'station', rentMult: 2 },
	},
	{
		id: 'ch_nearest_util',
		text: 'Advance to the nearest utility. If unowned, you may buy it. If owned, throw dice and pay 10× the amount rolled.',
	...
	{
		id: 'ch_jail_free',
		text: 'Get Out of Jail Free. Keep until used or sold.',
		effect: { kind: 'jailFree' },
	},
	...
	{
		id: 'ch_elected_chair',
		text: 'You have been elected Chair of the Board. Pay each player $50.',
...
function newChanceDeck() {
	return { draw: shuffled(CHANCE).map((c) => c.id), discard: [] };
}
function newChestDeck() {
	return { draw: shuffled(CHEST).map((c) => c.id), discard: [] };
}
