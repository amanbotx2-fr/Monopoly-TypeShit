...
const EFFECTS = {
	...
	click: () =>
		tone({ freq: 820, dur: 0.04, type: 'triangle', vol: 0.15, attack: 0.002, decay: 0.04 }),
	...
	move: () =>
		tone({ freq: 640, dur: 0.04, type: 'triangle', vol: 0.1, attack: 0.001, decay: 0.05 }),

	// Buy property — ascending two-tone "ka-ching".
	buy: () => {
		tone({ freq: 523.25, dur: 0.1, type: 'triangle', vol: 0.22 });
		setTimeout(
			() => tone({ freq: 783.99, dur: 0.22, type: 'triangle', vol: 0.22, decay: 0.28 }),
			80,
		);
	},
	...
	'money-in': () => {
		...
		setTimeout(
			() => tone({ freq: 1975, dur: 0.14, type: 'triangle', vol: 0.2, decay: 0.2 }),
			50,
		);
	},
	...
	'jail-escape': () => {
		...
		setTimeout(
			() => tone({ freq: 880, dur: 0.2, type: 'triangle', vol: 0.24, decay: 0.28 }),
			160,
		);
	},
	...
	'auction-end': () => {
		...
		setTimeout(
			() => noise({ dur: 0.12, vol: 0.3, filterFreq: 200, filterQ: 2, type: 'lowpass' }),
			140,
		);
	},
	...
	'trade-done': () => {
		...
		setTimeout(
			() => tone({ freq: 1320, dur: 0.22, type: 'sine', vol: 0.22, decay: 0.35 }),
			180,
		);
	},
	...
	'pass-go': () => {
		...
		setTimeout(
			() => tone({ freq: 880, dur: 0.28, type: 'triangle', vol: 0.26, decay: 0.35 }),
			180,
		);
	},
	...
	victory: () => {
		...
		notes.forEach((f, i) =>
			setTimeout(
				() => tone({ freq: f, dur: 0.25, type: 'triangle', vol: 0.3, decay: 0.3 }),
				i * 110,
			),
		);
		setTimeout(
			() => tone({ freq: 1319, dur: 0.6, type: 'triangle', vol: 0.32, decay: 0.7 }),
			500,
		);
	},
	...
	bankrupt: () => {
		...
		notes.forEach((f, i) =>
			setTimeout(
				() => tone({ freq: f, dur: 0.25, type: 'sawtooth', vol: 0.22, decay: 0.3 }),
				i * 180,
			),
		);
	},
	...
	error: () => {
		...
		setTimeout(
			() => tone({ freq: 175, dur: 0.18, type: 'square', vol: 0.18, decay: 0.22 }),
			60,
		);
	},
