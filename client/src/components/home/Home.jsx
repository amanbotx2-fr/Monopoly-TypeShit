...
export default function Home({ pushToast }) {
	...
	async function create() {
		...
		try {
			...
			pushToast(e.message || 'Failed to create');
		}
	}
	async function join(code) {
		...
		try {
			...
			return pushToast('Room not found');
		}
		nav(`/r/${trimmed}`);
	}
				...
				<div
					style={{
						...
						gridTemplateColumns: isMobile
							? '1fr'
							: 'minmax(360px, 1fr) minmax(300px, 420px)',
						gap: isMobile ? 16 : 24,
						alignItems: 'start',
					}}
				>
					<div className="card" style={{ padding: isMobile ? 20 : 28 }}>
						<div className="cluster" style={{ marginBottom: 18 }}>
							<span className="chip">
								<ShieldCheck size={13} /> Guest session
							</span>
							<span className="chip">
								<Map size={13} /> {boardCount || '...'} boards
							</span>
						</div>
						...
						>
							...
							{boards.community?.length > 0 && (
								<optgroup label="Community">
									{boards.community.map((b) => (
										<option key={b.id} value={b.id}>
											{b.name} — {b.authorUsername || 'Community'}
										</option>
									))}
								</optgroup>
							)}
							...
							{openRooms.length === 0 && (
								...
								>
									...
									<div className="empty-state-copy">
										Create a room and share the code when you are ready to bring
										friends in.
									</div>
								</div>
							)}
