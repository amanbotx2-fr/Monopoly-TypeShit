					...
					<div>
						...
						<div className="lobby-presence-card">
							...
							<div style={{ minWidth: 0, flex: 1 }}>
								<div
									style={{ color: 'var(--text)', fontWeight: 800, fontSize: 13 }}
								>
...
function SectionHead({ icon: Icon, label, count }) {
	return (
		<div className="section-title" style={{ marginBottom: 14 }}>
			...
			{count && (
				<div className="chip count" style={{ fontSize: 11 }}>
					{count}
				</div>
			)}
		</div>
	...
	return (
		...
		>
			...
			<div style={{ flex: 1, display: 'flex', alignItems: 'center', gap: 8, minWidth: 0 }}>
				...
				{!p.connected && (
					<span className="chip" style={{ fontSize: 10, color: 'var(--text-4)' }}>
						Disconnected
					</span>
				)}
			</div>
	...
	return (
		<div className="card">
			...
			<ToggleRow
				...
				value={r.freeParkingPot}
				disabled={!isHost}
				onChange={(v) => set('freeParkingPot', v)}
			/>
			<ToggleRow
				label="Auction on declined buy"
				value={r.auctionUnbought}
				disabled={!isHost}
				onChange={(v) => set('auctionUnbought', v)}
			/>
			<ToggleRow
				label="No rent while owner in jail"
				value={r.noRentInJail}
				disabled={!isHost}
			...
			<ToggleRow
				...
				value={r.randomTurnOrder}
				disabled={!isHost}
				onChange={(v) => set('randomTurnOrder', v)}
			/>
			<NumRow
				label="Jail fine"
				value={r.jailFine}
	...
	return (
		...
		>
			...
			<div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
				...
				<span
					className="mono"
					style={{ minWidth: 48, textAlign: 'center', color: 'var(--text)' }}
				>
					${value}
				</span>
				<button
