			...
			{expanded && (
				...
				>
					<div className="modal-header" style={{ padding: '12px 14px' }}>
						...
						<div className="modal-title" style={{ fontSize: 13 }}>
							Open trades
						</div>
					...
					<div style={{ maxHeight: 360, overflowY: 'auto', padding: 8 }}>
						{open.length === 0 && (
							...
							>
								No open trades. Hover a player and click the trade icon to propose
								one.
							</div>
						)}
						{open.map((t) => (
...
function TradeRow({ t, room, me, onOpen }) {
	...
	return (
		...
		>
			...
			>
				<span>
					{fromAccepted ? <b style={{ color: 'var(--success)' }}>✓</b> : '·'}{' '}
					{from?.username}
				</span>
				<span>·</span>
				<span>
					{toAccepted ? <b style={{ color: 'var(--success)' }}>✓</b> : '·'} {to?.username}
				</span>
				<div style={{ flex: 1 }} />
			...
			{expand && (
				...
				>
					<Bundle
						label={`${from?.username} gives`}
						b={t.offer}
						tiles={room.board.tiles}
					/>
					<Bundle
						label={`${to?.username} gives`}
						b={t.request}
						tiles={room.board.tiles}
					/>
				</div>
...
function Bundle({ label, b, tiles }) {
	...
	return (
		...
		>
			...
			{b.cash > 0 && (
				<div className="money" style={{ fontSize: 12 }}>
			...
			{jail > 0 && (
				<div style={{ fontSize: 11, color: 'var(--warning)' }}>Jail free × {jail}</div>
			)}
