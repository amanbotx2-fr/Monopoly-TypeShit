...
export default function PlayerHoverCard({ player, room, anchor }) {
	...
	const jailCards =
		(room.jailFreeLedger?.[player.userId]?.chance || 0) +
				...
				<div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
					<div
						style={{
							...
							fontSize: 13,
							fontWeight: 800,
							color: ['#FFFFFF', '#FACC15'].includes(player.color?.toUpperCase())
								? '#10130f'
								: 'var(--text)',
					...
					<div style={{ flex: 1, minWidth: 0 }}>
						...
						>
							...
							{player.bankrupt && (
								<span style={{ color: 'var(--danger)' }}>Bankrupt</span>
							)}
						</div>
					</div>
				</div>
...
function Stat({ label, value, color }) {
	return (
		<div style={{ flex: 1 }}>
			...
			<div
				className="mono"
				style={{ fontSize: 14, fontWeight: 700, color: color || 'var(--text)' }}
			>
				{value}
			</div>
		</div>
