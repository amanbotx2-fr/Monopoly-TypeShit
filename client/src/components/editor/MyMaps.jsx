			...
			<div className="page-shell">
				<header className="topbar" style={{ marginBottom: 22, flexWrap: 'wrap' }}>
					...
					<div>
						...
						<div style={{ color: 'var(--text-3)', fontSize: 13 }}>
							Manage custom boards saved on this browser identity.
						</div>
					</div>
					<div style={{ flex: 1 }} />
				...
				{!loading && filtered.length === 0 && (
					<div
						...
						style={{ display: 'grid', justifyItems: 'center', gap: 10, padding: 28 }}
					>
						<BrandLogo size={28} showText={false} />
						<div className="empty-state-title">No custom maps yet</div>
						<div className="empty-state-copy">
							Build a private board for friends or publish a map for other rooms to
							discover.
						</div>
						<button className="btn primary" onClick={() => nav('/editor')}>
							<Plus size={15} /> Create map
									...
									{renamingId === map.id ? (
										...
										>
											<input
												value={renameValue}
												onChange={(e) =>
													setRenameValue(e.target.value.slice(0, 80))
												}
												onKeyDown={(e) =>
													e.key === 'Enter' && saveRename(map)
												}
												style={{
													minWidth: 220,
													maxWidth: 420,
									...
									<button
										...
										onClick={() =>
											run(
												map.id,
												() =>
													api.updateBoard(map.id, {
														isPublic: !map.isPublic,
													}),
												map.isPublic ? 'Map unpublished' : 'Map published',
											)
										}
									>
