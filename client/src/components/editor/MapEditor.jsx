...
function normalizeTile(tile) {
	...
	if (t.type === 'property') {
		...
		t.mortgage = Number.isFinite(Number(t.mortgage))
			? Number(t.mortgage)
			: Math.floor(t.price / 2);
	...
	for (const [i, tile] of (board?.tiles || []).entries()) {
		...
		if (tile.type === 'property') {
			...
			if (!Array.isArray(tile.rent) || tile.rent.length !== 6)
				errs.push(`Property ${i} needs six rent values.`);
			if (!Number.isFinite(Number(tile.price))) errs.push(`Property ${i} needs a price.`);
			if (!Number.isFinite(Number(tile.houseCost)))
				errs.push(`Property ${i} needs a house cost.`);
		}
		if (
			(tile.type === 'station' || tile.type === 'utility') &&
			!Number.isFinite(Number(tile.price))
		) {
...
export default function MapEditor({ pushToast }) {
	...
	function pasteTile(pos) {
		...
		if (tileClipboard.type !== target.type)
			return pushToast('Paste requires the same tile type.');
	...
	async function save() {
		...
		try {
			...
			const editable = {
				...saved,
				groupColors: saved.groupColors || GROUPS,
				tiles: saved.tiles.map(normalizeTile),
			};
			setBoard(clone(editable));
			setBaseBoard(clone(editable));
	...
	const templateOptions = TEMPLATE_IDS.map((id) => presets.find((p) => p.id === id)).filter(
		Boolean,
	);
	...
	if (loading && !board) {
		return (
			<div className="app-page grid-bg" style={{ display: 'grid', placeItems: 'center' }}>
				<div
					className="card"
					style={{ display: 'grid', gap: 14, justifyItems: 'center', minWidth: 280 }}
				>
					<BrandLogo size={32} />
			...
			<div className="page-shell wide">
					...
					<div style={{ minWidth: 220 }}>
						...
						<div className="status-line" style={{ marginTop: 5 }}>
							<span
								className="dot"
								style={{
									background: dirty ? 'var(--warning)' : 'var(--success)',
									width: 8,
									height: 8,
								}}
							/>
				...
				{draftAvailable && (
					...
					>
						...
						<div style={{ flex: 1, color: 'var(--text-2)', fontSize: 13 }}>
							An autosaved draft is available.
						</div>
...
function TileRow({ tile, clipboard, onUpdate, onRent, onCopy, onPaste, onReset }) {
	...
	return (
		<tr style={{ borderTop: '1px solid var(--border)' }}>
			...
			<td style={cellStyle}>
				<span className="chip">{labelType(tile.type)}</span>
			</td>
			<td style={cellStyle}>
				<input
					value={tile.name || ''}
					...
					<input
						...
						onChange={(e) =>
							onUpdate(tile.pos, {
								...
								mortgage: Math.floor(Number(e.target.value) / 2),
							})
						}
			...
			<td style={cellStyle}>
				<div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
					...
					<button
						className="btn sm"
						disabled={!canPaste}
						onClick={() => onPaste(tile.pos)}
						title="Paste duplicated tile"
					>
						<ClipboardPaste size={13} /> Paste
					</button>
					<button
						className="btn sm ghost"
						onClick={() => onReset(tile.pos)}
						title="Reset tile"
					>
			...
			>
				...
				<div style={{ display: 'grid', gap: 12, padding: 20 }}>
					<label style={fieldLabel}>
						...
						<input
							value={mapName}
							onChange={(e) => setMapName(e.target.value.slice(0, 80))}
							autoFocus
						/>
					</label>
					<label style={fieldLabel}>
						...
						<textarea
							...
							onChange={(e) => setMapDescription(e.target.value.slice(0, 500))}
							rows={3}
							placeholder="Optional"
						/>
					</label>
